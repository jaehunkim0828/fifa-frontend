...
export default function RankUserResult({spid, name}: {spid: string, name: string}) {
    const [detail, setDedetail] = useState<any[]>([]);
    ...
    return (
        <div className={style.rankContainer}>
            {detail.map((e: {createDate: string, spPosition: number, status: PlayerRank}, i: number) => (
                <div key={i}>
                    ...
                    <div>골 성공율 :  {((e.status.goal/(e.status.shoot === 0 ? 1 : e.status.shoot)) * 100).toFixed()+ '%'}</div>
